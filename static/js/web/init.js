define(['jquery', 'bootstrap'], function ($, bs) {
	...
	$(document).on("click", '[data-toggle="ajaxModal"]',
			...
			function (e) {
				...
				var input = $('<input type="text" class="input-sm form-control" />');
				if (edit == 'textarea') {
	...
	$(document).on('click', '[data-toggle="selectCoupon"]', function () {
		$("#selectCoupon").remove();
		var _input = $(this).data('input');
		var _full = $(this).data('full');
		var _thisname=$(this).next("input");
		...
		var url = biz.url('util/selectcoupon', null, merch);
		if (_full) {
			url = url + "&full=1";
		}
		$.ajax(url, {
			...
			modal = $('<div class="modal fade" id="selectCoupon"></div>');
			$(document.body).append(modal), modal.modal('show');
			modal.append2(html, function () {
				$(document).off("click", '#selectCoupon nav').on("click", '#selectCoupon nav', function () {
					var couponid = $.trim($(this).data("couponid"));
					var couponname = $.trim($(this).data("couponname"));
					_thisname.val(couponid);
	...
	$(document).on('click', '[data-toggle="selectPrize"]', function () {
		$("#selectPrize").remove();
		var _input = $(this).data('input');
		var _full = $(this).data('full');
		var _thisname=$(this).next("input");
		...
		var merch = $(".diy-phone").data("merch");
		var url = biz.url('util/selectprize', null, merch);
		if (_full) {
			url = url + "&full=1";
		}
		$.ajax(url, {
			...
			modal = $('<div class="modal fade" id="selectPrize"></div>');
			$(document.body).append(modal), modal.modal('show');
			modal.append2(html, function () {
				$(document).off("click", '#selectPrize nav').on("click", '#selectPrize nav', function () {
					var couponid = $.trim($(this).data("couponid"));
					var couponname = $.trim($(this).data("couponname"));
					_thisname.val(couponid);
