...
$.device = (function() {
	...
	FoxUI.searchbar = {
		init: function() {
			$(document).on("focus", ".searchbar input",
			function(e) {
				var $input = $(e.target);
				$input.parents(".searchbar").addClass("searchbar-active")
			});
			$(document).on("click", ".searchbar-cancel",
			function(e) {
				var $btn = $(e.target);
				$btn.parents(".searchbar").removeClass("searchbar-active")
			});
			$(document).on("blur", ".searchbar input",
			function(e) {
				var $input = $(e.target);
	...
	var FoxUINumbers = function(element, params) {
		...
		self.params = $.extend(defaults, params || {});
		self.params.value = parseInt(self.container.data('value')) || parseInt(self.params.value);
		self.params.max = parseInt(self.container.data('max')) || parseInt(self.params.max);
		self.params.min = parseInt(self.container.data('min')) || parseInt(self.params.min);
		...
		self.$minus.removeClass('disabled'),
		self.$plus.removeClass('disabled');
		...
		self.checkNumber = function(num) {
			...
			//self.params.min=parseInt(self.params.min);
			...
			//self.params.max=parseInt(self.params.max);
	...
	var FoxUISwipe = function(swipe, params) {
		...
		var onTouchMove = function(e) {
			...
			allowItemClick = false;
			clearTimeout(self.interval);
			self.deltaX = self.deltaX / ((!self.activeIndex && self.deltaX > 0 || self.activeIndex == self.items.length - 1 && self.deltaX < 0) ? (Math.abs(self.deltaX) / self.width + 1) : 1);
		...
		var onTouchEnd = function(e) {
			...
			isTouched = false;
			var isValidSlide = Number(new Date()) - self.start.time < 250 && Math.abs(self.deltaX) > 20 || Math.abs(self.deltaX) > self.width / 2,
	...
	var FoxUIPullToRefresh = function(container, params) {
		...
		self.params = $.extend({},
		params || {});
		var isTouched, isMoved, touchStartX, touchStartY, isScrolling, touchesDiff, touchStartTime, refresh = false,
		loading = false,
		useTranslate = false,
		startTranslate = 0,
		...
		function handleTouchMove(e) {
			...
			if (touchesDiff > 0 && scrollTop <= 0 || scrollTop < 0) {
				if (scrollTop === 0 && !wasScrolled) useTranslate = true;
				if (useTranslate) {
					e.preventDefault();
	...
	var FoxUIPullToLoading = function(container, params) {
		...
		self.params = $.extend({},
		params || {});
		var isTouched, isMoved, touchStartX, touchStartY, isScrolling, touchesDiff, touchStartTime, loading = false,
		useTranslate = false,
	...
	Router.prototype._animateElement = function($from, $to, direction) {
		...
		var animPageClasses = ['fui-page-from-center-to-left', 'fui-page-from-center-to-right', 'fui-page-from-right-to-center', 'fui-page-from-left-to-center'].join(' ');
	...
	FoxUI.initPage = function(page) {
		...
		var $content = $page.hasClass('fui-content') ? $page: $page.find('.fui-content');
	...
	if (FoxUI.defaults.routerShowLoading) {
		$(window).on('pageLoadStart',
		function() {
			FoxUI.loader.show('mini')
		});
		$(window).on('pageAnimationStart',
		function() {
			FoxUI.loader.hide()
		});
		$(window).on('pageLoadCancel',
		function() {
			FoxUI.loader.hide()
		});
		$(window).on('pageLoadComplete',
		function() {
			FoxUI.loader.hide()
		});
		$(window).on('pageLoadError',
		function() {
			FoxUI.loader.hide()
		})
	}
	...
	$(document).on($.touchEvents.move, ".fui-header",
	function(e) {
		e.preventDefault()
	});
	$(window).on('pageInit',
