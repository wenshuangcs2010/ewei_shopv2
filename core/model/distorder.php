...
class Distorder_EweiShopV2Model
{
    ...
    function getTotals($merch = 0)
    {
        ...
        $totals['all'] = pdo_fetchcolumn(
            ...
            . " WHERE uniacid <>".DIS_ACCOUNT." {$condition} and ismr=0 and deleted=0", $paras);
        $totals['status_1'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('ewei_shop_order') . ""
            . " WHERE uniacid <>".DIS_ACCOUNT." {$condition} and ismr=0 and status=-1 and refundtime=0 and deleted=0", $paras);
        $totals['status0'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('ewei_shop_order') . ""
            . " WHERE uniacid <>".DIS_ACCOUNT." {$condition} and ismr=0  and status=0 and paytype<>3 and deleted=0", $paras);
        $totals['status1'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('ewei_shop_order') . ""
            . " WHERE uniacid <>".DIS_ACCOUNT." {$condition} and ismr=0  and ( status=1 or ( status=0 and paytype=3) ) and deleted=0", $paras);
        $totals['status2'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('ewei_shop_order') . ""
            . " WHERE uniacid <>".DIS_ACCOUNT." {$condition} and ismr=0  and status=2 and deleted=0", $paras);
        $totals['status3'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('ewei_shop_order') . ""
            . " WHERE uniacid<>".DIS_ACCOUNT." {$condition} and ismr=0  and status=3 and deleted=0", $paras);
        $totals['status4'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('ewei_shop_order') . ""
            . " WHERE uniacid<>".DIS_ACCOUNT." {$condition} and ismr=0  and refundstate>0 and refundid<>0 and deleted=0", $paras);
        $totals['status5'] = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('ewei_shop_order') . ""
            . " WHERE uniacid <>".DIS_ACCOUNT." {$condition} and ismr=0 and refundtime<>0 and deleted=0", $paras);
        return $totals;
    }
    ...
    function  get_tax($order_goods,$dispatch_price,$goodsprice,$alldeduct){
       ...
       // var_dump($order_goods);
        foreach($order_goods as $key=>$goods){
            ...
            $out_goods[$key]['price']=$goods['marketprice'];
    ...
    function get_dis_tax($order_goods,$address){
       ...
       // var_dump($order_goods);
        ...
        if($type){
            ...
            return array("disprice"=>$dispriceamount+$disprice_dispatch_price,'alltax'=>$alltax);
        }
        return 0;
    }
