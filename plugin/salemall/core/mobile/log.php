...
class Log_EweiShopV2Page extends MobilePage {
	...
	function getlist(){
        ...
        $list = array();
        if (!empty($total)) {
            $sql = 'SELECT log.id,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money,g.isverify,g.goodstype,log.addressid,log.storeid FROM ' . tablename('ewei_shop_creditshop_log') . ' log '
