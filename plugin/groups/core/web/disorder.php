...
class Disorder_EweiShopV2Page extends PluginWebPage {
	function main() {
		global $_W, $_GPC;
		$status = $_GPC['status'];
		...
		$list = pdo_fetchall("SELECT o.*,g.title,o.disgoods_id,g.category,g.thumb,g.groupsprice,g.singleprice,g.price as gprice,g.goodssn,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,
	...
	function detail(){
		...
		if($order['verifytype']==0){
			...
			if (!empty($verify['verifier'])) {
				...
				$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('ewei_shop_saler') . ' where openid=:openid  limit 1 ',
					array(':openid' => $verify['verifier']));
			}
		...
		if (!empty($order['address'])) {
			...
			if (!is_array($user)) {
				$user = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_member_address') . " WHERE id = :id", array(':id' => $order['addressid']));
			}
			$user['address'] = $user['province'] . ' ' . $user['city'] . ' ' . $user['area'] . ' ' . $user['address'];
	...
	protected function opData() {
		global $_W, $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_groups_order') . " WHERE id = :id", array(':id' => $id));
		if (empty($item)) {
			if ($_W['isajax']) {
				show_json(0, "未找到订单!");
			}
	...
	function delete() {
		global $_W, $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch("SELECT id,name FROM " . tablename('ewei_shop_groups_order') . " WHERE id = '$id'");
		if (empty($item)) {
	...
	function remarksaler() {
		...
		extract($opdata);
		if ($_W['ispost']) {
			pdo_update('ewei_shop_groups_order', array('remark' => $_GPC['remark']), array('id' => $item['id']));
	...
	function changeaddress() {
		...
		if (empty($item['addressid'])) {
			...
			if (!is_array($user)) {
				$user = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_member_address') . " WHERE id = :id", array(':id' => $item['addressid']));
			}
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				$item = pdo_fetch("SELECT id, orderno, address FROM " . tablename('ewei_shop_groups_order') . " WHERE id = :id", array(':id' => $id));
				...
				pdo_update('ewei_shop_groups_order', array('address' => $address_array), array('id' => $id));
