...
class Index_EweiShopV2Page extends PluginWebPage {
    ...
    function notstart(){
        ...
        if (empty($_GPC['search'])){
        $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
            " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.start_time )>".time()." AND goods.status = '0' AND g.status = '1' AND g.uniacid = :uniacid AND g.deleted = '0' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
        $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid']));
        ...
        $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid']));}
            ...
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
                " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.start_time )>".time()." AND goods.status = '0' AND g.status = '1' AND g.uniacid = :uniacid AND g.deleted = '0' AND g.title LIKE :keyword AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
            $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
    ...
    function complete(){
        ...
        if (empty($_GPC['search'])){
        $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
            " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<".time()." AND goods.status = '0' AND g.status = '1' AND unix_timestamp(goods.start_time )<".time()." AND g.uniacid = :uniacid AND g.deleted = '0' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
        $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid']));
        $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods').
            " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.uniacid = :uniacid AND g.deleted = '0' AND goods.account_id = :uniacid AND unix_timestamp(goods.start_time )<".time();
        $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid']));}
        else{
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
            ...
            $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
            $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods').
                " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.uniacid = :uniacid AND g.deleted = '0' AND g.title LIKE :keyword AND goods.account_id = :uniacid AND unix_timestamp(goods.start_time )<".time();
            $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
        }
        /*分页*/
        $pager = pagination($all,$page,$psize);
    ...
    function out(){
        ...
        if (empty($_GPC['search'])){
        $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
            " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE (g.status = '0' or g.deleted='1') AND goods.status = '0' AND g.uniacid = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
        $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid']));
        $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods').
            " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE (g.status = '0' or g.deleted='1') AND goods.status = '0' AND g.uniacid = :uniacid";
        $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid']));}else{
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
                " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE (g.status = '0' or g.deleted='1') AND goods.status = '0' AND g.title LIKE :keyword AND g.uniacid = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
            $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
            $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods').
                " goods INNER JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE (g.status = '0' or g.deleted='1') AND goods.status = '0' AND g.uniacid = :uniacid AND g.title LIKE :keyword";
            $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
        }
    ...
    function recycle(){
        ...
        if (empty($_GPC['search'])) {
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM" . tablename('ewei_shop_bargain_goods') .
                " goods INNER JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id WHERE goods.status = '1' AND g.uniacid = :uniacid ORDER BY goods.id DESC LIMIT " . ($page * $psize - $psize) . ",$psize";
            $onSell = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));
            $all_sql = "SELECT COUNT(*) FROM" . tablename('ewei_shop_bargain_goods') .
                " goods INNER JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id WHERE goods.status = '1' AND g.uniacid = :uniacid";
            $all = pdo_fetchcolumn($all_sql, array(':uniacid' => $_W['uniacid']));
        }else{
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM" . tablename('ewei_shop_bargain_goods') .
                " goods INNER JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id WHERE goods.status = '1' AND g.title LIKE :keyword AND g.uniacid = :uniacid ORDER BY goods.id DESC LIMIT " . ($page * $psize - $psize) . ",$psize";
            $onSell = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
            $all_sql = "SELECT COUNT(*) FROM" . tablename('ewei_shop_bargain_goods') .
                " goods INNER JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id WHERE goods.status = '1' AND g.title LIKE :keyword AND g.uniacid = :uniacid";
            $all = pdo_fetchcolumn($all_sql, array(':uniacid' => $_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
        }
        /*分页*/
        $pager = pagination($all,$page,$psize);
