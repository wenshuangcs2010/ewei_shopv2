...
define(['jquery'], function () {
    ...
    msg.prototype.auto = function (data, time) {
        ...
        if (parseInt(data.status) === 1) {
            return self.success(data.result.message, time, function () {
                !!data.result.url ? (window.location.href = data.result.url) : $.form.reload();
        ...
        self.error(data.result.message, 3, function () {
            !!data.result.url && (window.location.href = data.result.url);
        });
    };
