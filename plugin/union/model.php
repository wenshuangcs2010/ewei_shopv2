...
class UnionModel extends PluginModel
{
			...
			switch ($type) 
			{
				...
				switch ($type) 
				{
					...
					switch ($type) 
					{
						...
						switch ($type) 
						{
							case self: $datas = array('[订单编号]' => $params['logno'], '[付款金额]' => $params['money'], '[余额抵扣]' => $params['deduction'], '[付款时间]' => $params['paytime'], '[收银台名称]' => $params['union_title']);
							break;
							switch ($type) 
							{
								case self: $datas = array('[订单编号]' => $params['logno'], '[付款金额]' => $params['money'], '[余额抵扣]' => $params['deduction'], '[付款时间]' => $params['paytime'], '[收银台名称]' => $params['union_title']);
	...
	{
		global $_W;
		$wechat = $this->wechayPayInfo($_W['unionuser']);
	...
	{
		global $_W;
		return m('common')->AliPayBarcode($params, json_decode($_W['unionuser']['alipay'], true));
	}
	...
	public function pay($data, $auth_code = NULL) 
	{
		...
		if (($data['money'] <= 0) || ($data['paytype'] == 3)) 
		{
			...
			$data['paytime'] = time();
			pdo_update('ewei_shop_union_pay_log', $data, array('id' => $data['id']));
			m('member')->setCredit($data['openid'], 'credit2', -$data['deduction'], array(0, '收银台 ' . $_W['unionuser']['title'], '收款'));
			$user = $this->userInfo($data['unionid']);
	...
	public function updateOrder($log) 
	{
		...
		$user = $this->userInfo($log['unionid']);
		if ($log['paytype'] != '101') 
		{
		...
		{
			$log = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_union_pay_log') . ' WHERE `id`=:id and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $log['id']));
			if (empty($log['status'])) 
			{
	...
	public function paySuccess($log, $user) 
	{
		...
		if (!(empty($log['openid']))) 
		{
			...
			$this->sendMessage(array('logno' => $log['logno'], 'money' => $log['money'], 'deduction' => $log['deduction'], 'paytime' => time(), 'union_title' => $user['title']), self::PAY_CASHIER_USER, $log['openid']);
			$coupon = $this->seedCoupon($log['money'] + $log['deduction'], $log['openid']);
		}
		pdo_update('ewei_shop_union_pay_log', array('openid' => $log['openid'], 'payopenid' => $log['openid'], 'money' => $log['money'], 'status' => 1, 'paytime' => (0 < $log['paytime'] ? $log['paytime'] : time()), 'coupon' => $coupon), array('id' => $log['id']));
		$log['deduction'] = (double) $log['deduction'];
		if (!(empty($log['deduction']))) 
		{
			$userinfo = m('member')->getMobileMember($log['mobile']);
			m('member')->setCredit($userinfo['openid'], 'credit2', -$log['deduction'], array(0, '收银台 ' . $_W['unionuser']['title'], '收款'));
		}
		...
		{
			$operator = pdo_fetch('select * from ' . tablename('ewei_shop_union_operator') . ' WHERE id=:id AND unionid=:unionid limit 1', array(':id' => $log['operatorid'], ':unionid' => $log['unionid']));
		}
		if ($operator) 
		{
			$user['manageopenid'] = $operator['manageopenid'];
		}
		$this->sendMessage(array('logno' => $log['logno'], 'money' => $log['money'], 'deduction' => $log['deduction'], 'paytime' => time(), 'union_title' => $user['title']), self::PAY_CASHIER, $user['manageopenid']);
		...
		if (!(empty($log['openid']))) 
		{
			...
			{
				pdo_update('ewei_shop_union_pay_log', array('present_credit1' => $present_credit1), array('id' => $log['id']));
			}
		}
	...
	public function paytype($paytype = 0, $auto_code = NULL) 
	{
		...
		if (($paytype == -1) && ($auto_code !== NULL)) 
		{
			...
			{
				if (empty($_W['unionuser']['alipay_status']))
				{
					return error(-101, '暂时不支持支付宝支付!');
				}
				$paytype = 1;
		...
		if (empty($paytype) && !(empty($_W['unionuser']['wechat_status'])))
		{
			$paytype = 0;
		}
		else if ($paytype == '1') 
	...
	{
		global $_W;
		$userset = $this->getUserSet('', $_W['unionid']);
	...
	public function getOrderMoney($starttime = 0, $endtime = 0) 
	{
		...
		{
			$money = (double) pdo_fetchcolumn('SELECT SUM(money+deduction) FROM ' . tablename('ewei_shop_union_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND unionid=:unionid ', array(':uniacid' => $_W['uniacid'], ':unionid' => $_W['unionid']));
		}
		else 
		{
			$money = (double) pdo_fetchcolumn('SELECT SUM(money+deduction) FROM ' . tablename('ewei_shop_union_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND unionid=:unionid AND createtime BETWEEN :starttime AND :endtime', array(':uniacid' => $_W['uniacid'], ':unionid' => $_W['unionid'], ':starttime' => $starttime, ':endtime' => $endtime));
		}
		return $money;
	}
	...
	{
		global $_W;
		$set = $this->getUserSet('', $_W['unionid']);
	...
	public function createGoodsOrder($goods, $openid = '') 
	{
		...
		$order['paytype'] = 0;
		$order['merchshow'] = 0;
		$order['merchid'] = intval($_W['unionuser']['merchid']);
		$order['isparent'] = 0;
		$order['transid'] = '';
	...
	public function goodsCalculate($selfgoods = array(), $shopgoods = array(), $params, $return = false) 
	{
		...
		if (!(empty($selfgoods))) 
		{
			foreach ($selfgoods as $key => $val ) 
			{
				...
				$order['isgoods'] = 1;
				$data = array('unionid' => $order['unionid'], 'logid' => $order['id'], 'goodsid' => $val['goodsid'], 'price' => $val['price'] * $val['total'], 'total' => $val['total']);
				$g = pdo_get('ewei_shop_union_goods', array('unionid' => $data['unionid'], 'id' => $data['goodsid']));
				if (($g['total'] < $data['total']) && ($g['total'] != -1)) 
				{
					pdo_delete('ewei_shop_union_pay_log_goods', array('unionid' => $data['unionid'], 'logid' => $data['logid']));
					return array('res' => error('-101', $g['title'] . ' 库存不足, 剩余库存 ' . $g['total']));
				}
				pdo_insert('ewei_shop_union_pay_log_goods', $data);
			}
		}
		if (!(empty($shopgoods))) 
		{
		...
		{
			pdo_update('ewei_shop_union_pay_log', $order, array('id' => $order['id']));
			if ($return) 
			{
				return $order;
			}
	...
	public function setSelfGoodsStocks($logid = 0) 
	{
		...
		$goods = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_union_pay_log_goods') . ' WHERE unionid=:unionid AND logid=:logid', array(':unionid' => $_W['unionid'], ':logid' => $logid));
		foreach ($goods as $g ) 
		{
			pdo_query('UPDATE ' . tablename('ewei_shop_union_goods') . ' SET total=total-' . $g['total'] . ' WHERE unionid=:unionid AND id=:id AND total<>-1', array(':unionid' => $_W['unionid'], ':id' => $g['goodsid']));
		}
	}
	public function querycoupons($kwd = '') 
	{
		...
		$params[':merchid'] = intval($_W['unionuser']['merchid']);
		$condition = ' and uniacid=:uniacid and merchid=:merchid and (total=-1 OR total>0)';
		if (empty($_W['unionuser']['couponid']))
		{
			return array();
		}
		$condition .= 'AND id IN (' . $_W['unionuser']['couponid'] . ')';
		if (!(empty($kwd))) 
		{
