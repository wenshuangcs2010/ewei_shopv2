...
class Sysset_EweiShopV2Page extends UnionWebPage
{
	public function main() 
	{
		global $_W;
		global $_GPC;
		$id = intval($_W['unionid']);
		...
		if ($_W['ispost']) 
		{
			...
			{
				plog('union.user.add', '添加收银台 ID: ' . $res['id'] . ' 收银台名: ' . $res['title'] . '<br/>帐号: ' . $res['username']);
			}
			else 
			{
				plog('union.user.edit', '编辑收银台 ID: ' . $res['id'] . ' 收银台名: ' . $item['title'] . ' -> ' . $res['title'] . '<br/>帐号: ' . $item['username'] . ' -> ' . $res['username']);
			}
			$_SESSION['__union_' . (int) $_GPC['i'] . '_session'] = $res;
			show_json(1);
		}
	...
	public function userset() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			show_json(1, array('url' => unionUrl('sysset/userset', array('tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}
