...
class Log_EweiShopV2Page extends PluginWebPage {
	...
	public function main() {
		...
		$log = pdo_fetchall('select *  from ' . tablename('ewei_shop_creditshop_log') . "  where goodsid=:goodsid and status>=2 and uniacid=:uniacid  ",
				array(':goodsid' => '277', ':uniacid' => $_W['uniacid']));
	...
	protected function getData($type) {
		...
		$sql = "select log.*, m.nickname,m.avatar,m.realname as mrealname,m.mobile as mmobile, g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype,g.isverify,g.goodstype as iscoupon,
			s.storename,s.address as storeaddress,g.dispatch,g.goodstype from " . tablename('ewei_shop_creditshop_log') . " log "
	...
	function doexchange() {
		...
		if ($_W['ispost']) {
			if($goods['goodstype']==0){
				...
				/*$rbno = ('common')->createNO('creditshop_log', 'logno', 'RB');*/
				//红包参数
				$params = array(
					'openid'=>$log['openid'],
